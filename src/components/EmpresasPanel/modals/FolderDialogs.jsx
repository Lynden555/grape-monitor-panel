...
export const FolderContextMenu = ({ contextMenu, onClose, onRename, onDelete }) => (
  <Menu
  ...
  >
    <MenuItem
      onClick={() => onRename(contextMenu.folder)}
      sx={{
    ...
    <MenuItem
      onClick={() => onDelete(contextMenu.folder)}
      sx={{
        color: '#ef4444',
        fontSize: '14px',
...
export const CreateFolderDialog = ({
  open, onClose, newFolderName, setNewFolderName, onCreate
}) => (
  <Dialog open={open} onClose={onClose} PaperProps={{ sx: dialogPaperSx() }}>
    <DialogTitle sx={titleSx}>
      <Box sx={{
        ...
        <CreateNewFolderIcon sx={{ color: '#8b5cf6', fontSize: '20px' }} />
      </Box>
      Crear nueva carpeta
    </DialogTitle>
    <DialogContent>
    ...
    <DialogActions sx={{ px: 3, pb: 2.5, gap: 1 }}>
      <Button onClick={onClose} sx={cancelBtnSx}>Cancelar</Button>
      <Button
        onClick={onCreate}
...
export const RenameFolderDialog = ({ dialog, setDialog, onConfirm }) => (
  <Dialog
    open={dialog.open}
    onClose={() => setDialog({ open: false, folder: null, newName: '' })}
  ...
  >
    <DialogTitle sx={titleSx}>
      ...
      Renombrar "{dialog.folder?.nombre}"
    </DialogTitle>
    <DialogContent>
      <TextField
        autoFocus
    ...
    <DialogActions sx={{ px: 3, pb: 2.5, gap: 1 }}>
      <Button
        onClick={() => setDialog({ open: false, folder: null, newName: '' })}
        sx={cancelBtnSx}
      >
        Cancelar
      </Button>
...
export const DeleteFolderDialog = ({ dialog, setDialog, onConfirm }) => (
  <Dialog
    open={dialog.open}
  ...
  >
    ...
    <DialogContent>
      <Typography sx={{ color: 'rgba(255, 255, 255, 0.8)', mb: 1 }}>
        ¿Eliminar la carpeta <strong>"{dialog.folder?.nombre}"</strong>?
      </Typography>
    ...
    <DialogActions sx={{ px: 3, pb: 2.5, gap: 1 }}>
      <Button
        onClick={() => setDialog({ open: false, folder: null })}
        sx={cancelBtnSx}
      >
        Cancelar
      </Button>
